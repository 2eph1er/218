...
class BaykeShopSPUListView(ListView):
    ...
    def get_order_queryset(self, params, spus, filter={}):
        ...
        # 按销量排序
            ...
            datas = [{'spu': spu, 'sales': spu.baykeshopsku_set.aggregate(Sum('sales'))['sales__sum']} for spu in spus
                     if spu]
...
class BaykeShopCategoryDetailView(SingleObjectMixin, BaykeShopSPUListView):
    ...
    def get_queryset(self):
        ...
        if cate.parent is None:
            spus = BaykeShopSPU.objects.filter(category__in=cate.baykeshopcategory_set.all()).order_by(
                '-add_date').distinct()
            # 按销量或价格排序
            if self.get_params_filed(params):
                cates = cate.baykeshopcategory_set.all()
                spus = self.get_order_queryset(self.get_params_filed(params), spus=spus,
                                               filter={'spu__category__in': cates})
        else:
            spus = BaykeShopSPU.objects.filter(category__id=self.kwargs['pk']).order_by('-add_date')
            # 按销量或价格排序
            if self.get_params_filed(params):
                spus = self.get_order_queryset(self.get_params_filed(params), spus=spus,
                                               filter={'spu__category__id': self.kwargs['pk']})
        return spus
...
class BaykeShopSPUDetailView(DetailView):
    ...
    def get_skus(self):
        ...
        bh = BrowseHistory.objects.filter(user_id=BaykeUserInfo.objects.get(id=self.request.user.id)
                                          , spu_id=BaykeShopSPU.objects.get(id=spu.id))
        if bh:
            bh.update(add_date=timezone.now())
        else:
            BrowseHistory.objects.create(
                user_id=BaykeUserInfo.objects.get(id=self.request.user.id),
                spu_id=BaykeShopSPU.objects.get(id=spu.id))
